...
public class OldNoiseGeneratorOctaves extends cs {
  private OldNoiseGeneratorPerlin[] generatorCollection;
  
  private int octaves;
  
  public OldNoiseGeneratorOctaves(Random random, int i) {
    this.octaves = i;
    this.generatorCollection = new OldNoiseGeneratorPerlin[i];
    for (int j = 0; j < i; j++)
      this.generatorCollection[j] = new OldNoiseGeneratorPerlin(random); 
  }
